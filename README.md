...
| 0.4.0 | Scoring on subject trajectories: the window, decay and thresholds chosen from recorded trajectories and a multi-turn dataset (shared with the `abuse` template, which gets its own dataset at the same time) |
