...
export async function evaluate(req: Req, ctx: Ctx): Promise<verdict.Verdict> {
  ...
  if (r === rulesMod.BLOCK) {
    ...
    return verdict.newVerdict({ action, verdict: verdict.MALICIOUS, score: 1, source: verdict.SRC_L1, reason });
  }
  ...
  if (ctx.breaker && !(await ctx.breaker.allow())) {
    ...
    return verdict.newVerdict({ action, verdict: label, async, source: verdict.SRC_BREAKER, reason: "breaker open", fingerprint: fp });
  }
  ...
  if (!prompt) {
    ...
    return verdict.newVerdict({ action, verdict: label, async, source: verdict.SRC_L2, reason: perr, fingerprint: fp });
  }
...
export { rulesMod as rules, normalize, judge, policy, verdict };
