...
// Well-formed escapes still reach the adapter as sent: UTF-8, an escaped
// control character, and escapes of bytes that are not UTF-8 (the overlong
// %C0%AE, %FF), which nginx, Envoy and HAProxy take and jev-edge judges as
// sent. The query string is not the path and is not checked.
