...
`denoKvStore(kv, prefix?, clock?, { consistency? })` is a `Store` over Deno KV, typed structurally (no dependency on Deno's types). ttl becomes `expireIn`, and reads check the expiry themselves because Deno KV deletes expired keys lazily. `incr` and `expire` are check-and-set loops on `kv.atomic()`, so the subject ring counter is atomic across isolates and regions; the breaker's counters use plain get / set and can lose an increment under contention. `getMany` reads up to 10 keys per `kv.getMany` round trip; the subject ring loads its slots with it. `consistency: "eventual"` makes plain reads use the nearest replica: `denoHandler` sets it for the verdict cache (a stale read at worst judges a text again) and keeps strong reads for the breaker state and the subject store; the check-and-set loops always read strong. Without `kv` every store is memory per isolate.
...
`evaluate(request, rt)` returns the verdict without forwarding; both take an optional `{ waitUntil }` (a Workers `ExecutionContext`) so the subject write outlives the response. `GET /_jev/health` is served by `handle` unless `health: false`, and answers `{ ok, adapter, core }`: it is on the traffic path, reachable by anyone, so the provider, model, mode and endpoint (a thin Worker's origin) are left out unless `health: "details"`.
...
**Subjects.** `config.subject = { enabled: true, from: "cookie", name: "sid", salt: env.JEV_SUBJECT_SALT }` gives every request a hashed subject id (`<from>:` + SHA-256 hex over `salt \0 value`; the raw cookie or header is never stored; a salted value is hashed up to 64 KiB, and one past it is dropped with a warning once per isolate) and records its trajectory in `subjectStore` (KV, the `JevState` Durable Object, or memory), `max_entries` per subject for `history_ttl`. The OpenResty adapter uses SHA-256 as well since 0.3.1, so the same salt and value give the same id on both. The id is forwarded upstream as `X-Jev-Subject` (the thin Worker also sends it with the `/_jev/authz` call); configure the origin with `subject = { enabled = true, from = "header", name = "x-jev-subject", hashed = true }`. With `hashed = true` only values of the form `<letters>:<hex>` (at most 512 bytes) are accepted as ids; anything else is ignored. A cookie subject is every value the origin's framework may read (a duplicate name, a quoted value), and any blocked one blocks; an `Authorization` subject is keyed on the credential, the scheme lowercased, so respelling it gives no fresh subject.

**The origin must take `X-Jev-Subject` from the Worker only.** With `hashed = true` it trusts the header verbatim, so a client that reaches the origin directly picks its own subject, or none. Put the Worker's `/_jev/authz` calls on the gateway-only listener of [example.nginx.conf](../openresty/conf/example.nginx.conf) and set `originToken` (or `JEV_ORIGIN_TOKEN`): the Worker sends it as `X-Jev-Origin-Token` and that listener refuses a call without it. The requests the Worker forwards reach the origin's ordinary server without the token: admit only the Worker there (Cloudflare Authenticated Origin Pulls or mTLS, or a firewall that admits only Cloudflare's ranges), or clear `X-Jev-Subject` on that server before `access()` and leave reputation to the authz calls. The origin still judges every request; only reputation depends on this ([Subject reputation](../../docs/design.md#subject-reputation)).

The trajectory uses the same ring layout as the Lua core: one counter per subject (`subj:<id>:n`, bumped with the store's `incr`) and one key per entry (`subj:<id>:<slot>`), so concurrent requests from one subject do not overwrite each other's entries. That needs a store with an atomic `incr`: the memory store (per isolate) and the `JevState` Durable Object have one. Pass `subjectStore: env.JEV_STATE` (or `{ namespace, name }`): each subject then lives in a Durable Object of its own (`jev-subject:<id>`), so every increment is atomic across isolates and locations and no subject queues behind another. A judged request makes one hop to it before the judge (the trajectory, the running block and the window's points together) and one after it, through `waitUntil`; a request no rule would judge does not ask it. The presets pass `env.JEV_STATE` as the subject store when `subject.reputation` is on and the options name none. An object keeps a subject's entries until they are overwritten, so storage grows with the number of distinct subjects. `durableStore(stub)` of a single stub keeps every subject in that one object. A stub kept past its request falls back as for `state`: to isolate memory, where an increment is atomic within the isolate only. KV has no increment, so its `incr` is a get and a put: best effort, and two isolates writing for the same subject at once can still lose an entry. A custom `Store` without `incr` keeps the older one-list-per-subject layout (`subj:<id>`), a read-then-write with the same caveat.
