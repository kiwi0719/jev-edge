...
Pass `state` the Durable Object namespace, `env.JEV_STATE`, not a stub. workerd binds a stub to the request that made it, so the runtime makes its own per operation (`idFromName("jev-edge")` is a hash and `get` no round trip); that is what lets the runtime above live at module scope, and it is what `fullWorker` and `pagesMiddleware` pass. A stub is still accepted, for a runtime built per request. Kept across requests, it fails with "Cannot perform I/O on behalf of a different request": the runtime then logs one `console.error` naming this fix and keeps breaker and adaptive state in isolate memory, as without the binding, instead of failing open on every later request. `state` is taken for a namespace when it has `idFromName` and `get` and no `fetch`, for a stub when it has a `fetch` method (a real stub answers every property name, so nothing else tells it apart), and for a `Store` otherwise.
