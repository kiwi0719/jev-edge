...
1. Install the Lua sources. The rock installs everything, the plugin included (`kong.plugins.jev-edge.handler` and `schema`, next to `jev.core`, `jev.rules.*` and `resty.jev.*`), into Kong's own tree, so no `KONG_LUA_PACKAGE_PATH` is needed:

   ```bash
   luarocks install lua-resty-jev-edge
   ```

   Or from a checkout, with every directory on the path (the trailing `;;` keeps Kong's own path):
   ...
   Use one or the other, not both: the first match on the path wins, so a checkout's handler on the path would run against the rock's core, from two different versions. `lua-resty-http` ships with Kong.
...
2. Enable the plugin and declare the shared dict and the key's environment variable, in `kong.conf` or as `KONG_*` variables:
   ...
   Kong turns each `nginx_http_<directive>` into one `<directive> <value>;` line, so the dicts for `subject` ride on the same value: `"jev_cache 64m; lua_shared_dict jev_subject 16m; lua_shared_dict jev_subject_rep 4m"` (`jev_subject_rep` holds subject reputation apart from the trajectories, which never evict and drop new entries when full). A custom nginx template works too. Instead of the environment variable, `jev.api_key` is referenceable: `api_key: "{vault://env/typesafe-api-key}"`. A rotated vault secret (the key or `subject.salt`) is picked up on the next request Kong resolves it for, and a reference that resolves to nothing falls back to `jev.api_key_env` with a warning.

3. Start Kong with `TYPESAFE_API_KEY` in its environment and add the plugin to a route or service (above).

**Watch the path the client sends.** `watch_paths` match the client-facing path (`ngx.var.uri`, decoded), before `strip_path` or any rewrite. On a route that serves an LLM under a prefix of its own (`/openai-chat`, `/llm/v1/...`, a route paired with `ai-proxy`), the default `llm-endpoints` paths match nothing and every request passes as "path not watched". Scope a rule to the route instead, and let the route decide what reaches the plugin:

```yaml
rules_json: '[{"id":"route","extends":"llm-endpoints","watch_paths":["^/"]}]'
```
...
One runtime (provider client, breaker, adaptive timeout) is built per plugin conf table and kept until Kong hands over a new one (config change, declarative reload) or the resolved key or salt changes. Breaker, adaptive timeout and in-flight counters are keyed by provider, endpoint, model, a hash of the key, `max_inflight` and the breaker settings, so plugin instances share them only when all of those match: a route with a revoked key opens its own breaker, not its neighbours'. Verdict-cache keys are scoped by rule, templates, deployment context, provider, model, and when set the endpoint and question wording (`core.cache_key`).
...
Real Kong (`kong:3.9`, DB-less, declarative [e2e/kong.yml](e2e/kong.yml)) in front of a stub app, mock provider, about fifty checks: skipped / safe / suspicious / blocked, a block that carries no score, reason or source, no client `X-Jev-*` upstream, a subject behind `key-auth` with `hide_credentials`, a rotated vault key, one route's revoked key not opening another route's breaker, the schema fields, `/v1%2Fchat/completions` (and `%2f`) judged like `/v1/chat/completions`, block body and headers, inbound header stripping, fail-open on provider failure, GET at L1, gzip bodies decoded and judged, a body spooled to disk read and blocked, an inline tenant rule through `rules_json`, and the decision log line. Then hybrid mode ([e2e/hybrid/](e2e/hybrid/)): a control plane on PostgreSQL and a DB-less data plane that lacks a rule set the control plane has; the data plane keeps syncing and answers the routes naming it with verdict `error`. Configs in [e2e/](e2e/).
