...
Real Kong (`kong:3.9`, DB-less, declarative [e2e/kong.yml](e2e/kong.yml)) in front of a stub app, mock provider, eighteen checks: skipped / safe / suspicious / blocked, block body and headers, inbound header stripping, fail-open on provider failure, GET at L1, gzip bodies decoded and judged, a body spooled to disk read and blocked, an inline tenant rule through `rules_json`, and the decision log line. Configs in [e2e/](e2e/).
