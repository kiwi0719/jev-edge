...
Under steady traffic the gateway's adaptive timeout sits at its floor, `timeout_ms`. A request that takes longer passes unjudged and counts toward the breaker, so the floor has to cover the worst case, not the typical request. The client also chooses how many worst-case texts it sends at once, up to the gateway's `max_inflight`. On CPU, N of them at once take about N times as long as one, or wait for a worker and get a 503. The profile sets 500 / 800 ms: about 2.5 times 6 windows at the ~33 ms per window Laya is reported at on CPU, for one at a time, which is what its `max_inflight = 1` sends. Measure your own build at your `max_inflight` (below). If the worst case does not fit the latency you can add, lower `max_judge_bytes`, add CPUs, or use a GPU (`LAYA_ORT_PROVIDERS`), where the windows of a batch run in parallel.

## max_inflight sheds a burst before the server must

The gateway's `jev.max_inflight` is how many L2 calls one gateway makes at once, all its nginx workers together. A request past it is not sent: it passes as an L2 error with the reason `max_inflight exceeded`, which does not count toward the breaker. What this server cannot answer in time does count: its `503 overloaded`, and a timeout at the gateway. Once half the calls in the breaker's window fail, L2 is off for every tenant for `breaker.open_s`. So `max_inflight`, summed over the gateways that call one server, must be at most what the server answers at once in time when every call is the worst case, since the client picks both the text and how many it sends. Then a burst is shed at the gateway, and never by this server.

With this server's defaults that is one, and the profile sets `max_inflight = 1`. From 2 CPUs on, the default pool has 2 workers or more (the start line says how many), so a short text is not stuck behind a long one. The workers share one pool of onnxruntime threads, though, so two worst-case texts at once each take about 1.5 to 2 times as long as one: from ~200 ms at the reported ~33 ms per window to past the 300 ms the gateway reads at a 500 ms floor. A call the gateway gave up on still holds its worker when the next call comes, and that one gets a 503.

Measured on a 10-CPU host with a MiniLM-shaped model at ~30 ms per window (6 windows, ~170 ms for the worst case alone), this server's defaults, and the real gateway running the profile in enforce mode with a deployment context: 64 distinct worst-case texts at once, in rounds, then short attacks one at a time.

| `max_inflight` | this server | the gateway's L2 calls | the breaker |
|---|---|---|---|
| 64 (before) | 78 answered `503 overloaded` | 1 judged, 31 `laya http 503`, 32 `timeout`, then `breaker open` | open: the short attacks after passed unjudged |
| 2 | 2 answered 503 | 5 of 32 failed (3 `timeout`, 2 `laya http 503`): two at once took up to 320 ms, and the gateway reads for 300 | closed, with failures on the record |
| 1 (the profile) | no 503 | every call answered in time, at most 220 ms; the rest passed as `max_inflight exceeded` | closed: the short attacks after were judged and blocked |

One at a time costs throughput: a request that comes while another is being judged passes unjudged, and on CPU a short text takes a few ms. To raise `max_inflight`, add capacity (CPUs, `LAYA_WORKERS`, or a GPU through `LAYA_ORT_PROVIDERS`), then run `make conformance` with `CONCURRENCY` at the value you want (step 3 below). It passes when that many worst-case texts at once are answered in time, and otherwise prints how many are. `LAYA_BACKLOG` must be at least the sum over the gateways.
...
3. **Check the protocol and the latency** on the hardware you will run:
   ```bash
   make conformance ENDPOINT=http://laya-server:8080/v1/systemone API_KEY=change-me STRICT=1 BUDGET_MS=500 CONCURRENCY=1
   ```
   `BUDGET_MS` is the `timeout_ms` you plan to run, and the server's `LAYA_GATEWAY_TIMEOUT_MS` should be the same. `CONCURRENCY` is the `max_inflight` you plan to run, summed over the gateways that call this server (64 when left out, the jev default). The suite times a short text and the worst case: `max_judge_bytes` of text at one token per byte, in the deployment-context wording, first one at a time and then `CONCURRENCY` at once, like the gateway at `max_inflight`. It also opens `CONCURRENCY` connections at once. A timing check passes only when its p99 is at most half of `BUDGET_MS`, because the gateway reads for 60% of it and the tail needs room. Every worst-case request must get a 200: a 503 is an L2 error too. The run ends with a `timeout_ms:` line, 2 to 3 times the worst-case p99 at `max_inflight`. When the server cannot answer that many at once in time, the line gives the `max_inflight` it can. Set the profile's `timeout_ms` and `max_inflight` from that line, never from the short-text p99. For a profile with a different `max_judge_bytes`, `max_inflight` or `deployment_context`, run `python3 conformance/run.py` with `--judge-bytes`, `--concurrency` and `--assistant`.
