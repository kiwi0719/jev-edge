...
| `LAYA_QUEUE_MS` | `50` | how long a request waits for a free worker before `503 overloaded`. At most the gateway's read wait, 60% of `timeout_ms`, minus the worst-case p99 (below) |
...
- **The 503 comes while the gateway still reads.** The gateway waits for the answer 60% of its L2 timeout (connect 30%, send 10%, read 60%): 300 ms at the profile's floor of 500 ms. A 503 sent after that reaches no one, the gateway logs a timeout instead, and a request scored after that uses a worker for an answer no one reads. `make conformance` passes a worst case of up to half of `timeout_ms`. A request that gets a worker at the end of its wait is then still answered in time only if the wait is at most 60% − 50% = 10% of `timeout_ms`. That is the default of 50 ms. For another `timeout_ms`, or a server faster than that, set `LAYA_QUEUE_MS` to 60% of `timeout_ms` minus the worst-case p99 that `make conformance` reports.
