...
- **A multipart pass-through request** (a file upload through `/openai/v1/files`) reaches the hook without its body on 1.102.1, which does not parse the form there, and with the form's fields on 1.80.11.
...
- **A Gemini body** (`generateContent`, and the `/gemini` and `/vertex_ai` pass-through) keeps Gemini's own shape: `tools`, `systemInstruction` and `contents`, as the client sent them. jev-edge's default rules read `systemInstruction.parts`, `system_instruction.parts` and `contents[*].parts`; jev-edge 0.6.1 and earlier do not, and neither do rules that set their own `text_fields` without them, so add those paths there, or a Gemini request is judged as having no text.
- **Media** is left out: image, audio and file parts keep only their `type` (and any `text`), and base64 sources, inline data and Bedrock's `bytes` are removed. Tool-call arguments are the exception, sent whole like the tool definitions: OpenAI's `tool_calls[*].function.arguments` and `custom.input` and `function_call.arguments`, Anthropic's `tool_use` `input`, Bedrock's `toolUse.input`, a Responses `function_call`'s `arguments` and a custom tool call's `input`, Gemini's `functionCall.args`. jev-edge reads every key and string in them, so a `bytes` or `image_url` key there is text the model reads. The model, sampling parameters and LiteLLM's metadata are not sent.
