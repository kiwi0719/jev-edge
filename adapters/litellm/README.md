...
5. Set jev-edge's `client_ip.trusted_hops` to the number of proxies in front of LiteLLM that append the address they saw to `X-Forwarded-For` (nginx's `$proxy_add_x_forwarded_for`, a load balancer), plus one for LiteLLM itself: the guardrail appends the peer LiteLLM saw, as a proxy does. Keep LiteLLM's `use_x_forwarded_for` off ([details](#what-it-sends)).

   | Clients reach LiteLLM | `client_ip.trusted_hops` |
   |---|---|
   | directly | 1 |
   | through N proxies that append to `X-Forwarded-For` | N+1 |
...
- **Media** is left out: image, audio and file parts keep only their `type` (and any `text`), and base64 sources, inline data and Bedrock's `bytes` are removed. Tool calls and tool results the model reads as JSON are the exception, sent whole like the tool definitions, since a `bytes` or `image_url` key there is text the model reads: OpenAI's `tool_calls[*].function.arguments` and `custom.input` and `function_call.arguments`, Anthropic's `tool_use` `input`, a Responses `function_call`'s `arguments` and a custom tool call's `input`, which jev-edge's rules for tool-call arguments read key and string alike; Gemini's `functionResponse.response` (`function_response` in snake_case), which jev-edge reads whole as a tool result; and Bedrock's `toolUse.input`, a Bedrock `toolResult`'s `text` and `json` blocks and Gemini's `functionCall.args` (`function_call`), which jev-edge reads too (the `toolResult` as a tool result). A part keeps them whatever `type` it has, since LiteLLM's `generateContent` adapter reads a function response next to a `type` of `image`. The media in a function response's own `parts` and in a `toolResult`'s image and video blocks is left out, and so are the bytes of a Bedrock Converse document of a binary format (pdf, doc, docx, xls, xlsx). A Converse document of a text format (txt, md, csv, html), in a message or in a `toolResult`, is text the model reads, so it is read: its `source.bytes` (base64, at most `JEV_EDGE_MAX_BODY_BYTES`) decoded as UTF-8, or its `source.text` or `source.content`, sent as a text part right after the block that holds it. One the guardrail cannot read (not base64, over that bound, an `s3Location`) is left out: the rest of the request is judged all the same, and the verdict notes the document ([below](#what-it-does-with-the-answer)). The model, sampling parameters and LiteLLM's metadata are not sent.
...
- **`X-Forwarded-For`** is the `X-Forwarded-For` header LiteLLM's proxy received (from the `proxy_server_request` the proxy builds, never from body or metadata fields a client sends) with the proxy's own `requester_ip_address` appended, one hop, as nginx appends `$remote_addr` (the address of the peer on 1.102.1; on 1.80.11 only with a premium licence), or that address alone when no header came in. jev-edge's `client_ip.trusted_hops` counts from the right: 1 takes the peer LiteLLM saw, N+1 the address the first of N appending proxies in front of it saw (step 5 of the install), never an entry the client wrote. Without a peer (1.80.11 without the licence), or with `use_x_forwarded_for` on, where `requester_ip_address` is the client's own header, the hop appended is `unknown`, so the count stays the same: reached directly, such requests share that one address in jev-edge's reputation. With neither a header nor a peer, nothing is sent and jev-edge counts every such request against LiteLLM's own address, as it does for the generic pass-through routes (LiteLLM records no address for them) and for `/guardrails/apply_guardrail` on 1.80.11. A batch file's lines and a realtime session's messages carry the address of the upload or the session.
...
- Every request gets `jev_verdict` with `verdict`, `score`, `source`, `reason`, `action` and, from jev-edge, `request_id` (and `unjudged` when a part of the request was not judged, below), in the metadata LiteLLM keeps for itself: `metadata`, or `litellm_metadata` on the routes where the API has a `metadata` parameter of its own (Responses, Anthropic messages, batches, files, assistants, and Bedrock on 1.102.1); the client's `metadata` there is sent on to the provider and never written. On a generic pass-through, whose data is the client's body itself, it goes in that body's `metadata`, which LiteLLM takes out before it sends the body on. The same verdict is recorded as LiteLLM's standard guardrail information (status `success`, `guardrail_intervened` for a block, `guardrail_failed_to_respond` when jev-edge could not be asked), which logging callbacks receive whole (`standard_logging_object.guardrail_information`). The spend logs' `guardrail_information` records the guardrail and its status; the verdict itself (`guardrail_response`) is there on 1.80.11, but 1.102.1 writes `REDACTED_BY_LITELM` unless `store_prompts_in_spend_logs` is on. `jev_verdict` is not a spend-log field.
- With `JEV_EDGE_ENFORCE=true`, a block from jev-edge (status >= 400 with `X-Jev-Verdict`, 403 by default) raises `HTTPException(<that status>, {"error": "request rejected", "request_id": "<X-Jev-Request-Id>"})` and the call never reaches the model. The client never sees the score, the reason or the source (they would tell an attacker how close a prompt came and which pattern fired); the whole verdict is in the request's metadata as `jev_verdict`, in the guardrail log, and on the exception as `jev_verdict`. With `false` the request continues annotated; use this for the monitor week and read the scores from the logs.
- **Unjudgeable**: an answer without `X-Jev-Verdict` below 500, other than 429, means the server in front of jev-edge refused the request before jev-edge ran (413 past `client_max_body_size`, 400 or 431 for headers, 414, a 404 from something that is not jev-edge). It is annotated `skipped`, source `adapter`, reason `unjudgeable: authz answered <status>`, and passed or blocked with 403 as `JEV_EDGE_UNJUDGED` says. A Bedrock text document the guardrail cannot read ([above](#what-it-sends)) is a part nobody judged, and attaching one hides nothing: the rest of the request is judged as usual, and its verdict gets `unjudged: "unjudgeable: call type <name>: bedrock document <format> not readable"`. With `JEV_EDGE_UNJUDGED=block` a request jev-edge passed, or could not be asked about, is blocked with 403 for that document; a block stays jev-edge's. A request with no text beside such documents is unjudgeable, with that reason, without a round trip. `JEV_EDGE_UNJUDGED` is the counterpart of the `-unjudged` flag of the [Envoy gRPC shim](../envoy/README.md) and the [HAProxy agent](../haproxy/README.md): the same values and default.
- **Fail-open**: connection errors, timeouts, and a 5xx or a 429 without the header (the judge, or a proxy or rate limiter in front of it, unavailable) annotate `verdict: error` and let the call through, whatever `JEV_EDGE_UNJUDGED` says (a Bedrock text document the guardrail cannot read is still passed or blocked as it says, above). Requests without text are `skipped`, source `adapter`, reason `no text`, without a round trip. A prompt sent as token ids (`"prompt": [40, 1541, ...]`, or lists of them) is not "no text": it goes to jev-edge, which reports it `unjudgeable: token prompt`, and a rule's `token_prompts = "block"` refuses it in enforce mode.
