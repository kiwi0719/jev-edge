...
- Chat requests: `{"messages": [...]}` with the string contents of every message (text parts of multimodal messages are kept, images dropped). Completion requests: `{"prompt": "..."}`. `X-Forwarded-For` carries the caller's IP from LiteLLM's `requester_ip_address` metadata (without it, the request's whole `X-Forwarded-For` chain, so jev-edge's `client_ip.trusted_hops` picks the hop instead of the client-forgeable first entry), so jev-edge's reputation and L3 work per client, not per proxy.
