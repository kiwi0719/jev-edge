...
`BUDGET_MS` is the L2 timeout floor the gateway runs with, `jev.timeout_ms`. `run.py` also takes `--judge-bytes` (default 4096, the profile's `max_judge_bytes`), `--concurrency` (default 64, `jev.max_inflight`: the burst and the worst case at once) and `--assistant` (your `deployment_context`) for the load and worst-case checks below.
...
- **Headroom**: every timing check passes only with 2x headroom, a p99 of at most half of `BUDGET_MS`. The gateway splits its L2 timeout into connect 30%, send 10% and read 60%, so at half the budget its read wait is still 1.2 times the p99, room for the tail past it.
- **Load**: 64 new connections at once (`--concurrency`), as the gateway opens them at `max_inflight` when its keepalive pool is cold. Each must connect within the gateway's connect budget (30% of `BUDGET_MS`) and get a 200, the slowest with the headroom. A server whose listen backlog is smaller than the burst drops connections. The client retries a dropped SYN only after about a second, so the gateway sees a timeout, passes the request, and counts a breaker failure.
- **Latency**: p99 over 50 sequential short requests, with the headroom. Then the **worst case**: the longest text the gateway sends (`--judge-bytes`), made of punctuation and rare letters so that it costs about one token per byte, in the deployment-context wording. It is sent 50 times one at a time, then `--concurrency` at once in rounds, on warm connections: the client chooses both the text and how many it sends at once, up to `max_inflight`. Every request must get a 200, because a 413 for a text the gateway sends is a bypass, and so is a 503 from a server out of workers. A 503 that comes after the gateway stopped reading (60% of `BUDGET_MS`) fails on its own: the gateway logs a timeout instead. On CPU a text split into N windows costs about N model calls, so the worst case can be tens of times the short one, and several of them at once take several times as long again. Under steady traffic the gateway's adaptive timeout sits at its floor, so the floor has to cover the worst case at `max_inflight`. The run ends with a `timeout_ms:` line: 2 to 3 times the worst-case p99 at `--concurrency`, or, when the server cannot answer that many in time, what one at a time needs and the `max_inflight` the server holds. Set the floor and `max_inflight` from that line, not from the short-text p99.
