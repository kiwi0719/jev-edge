...
| 下一个版本（计划中） | 对 0.6.1 全面审计后的修复（见 [CHANGELOG](../CHANGELOG.md) 的 Unreleased）：L1 读到后端读到的内容（所前置的各个服务的路由和别名、大小写折叠后的路径和键名、解码器拒收的 JSON、按 RFC 2046 读取的 multipart、token id 提示词、压缩过的和超大请求体的结尾）；判定工具调用参数和工具定义；熔断器只统计 provider 自身的失败；只有主体自己的文本才计信誉分；拦截响应只告诉客户端判定结果和请求 id；遇到转发不了的内容就失败放行的中继，现在改为拒绝或标注；Kong hybrid 模式、APISIX 的 etcd 密钥和执行顺序、跨请求使用的 Durable Object；加固 release-npm。 |
...
7. **正则预筛**，作用于抽取出来的全部文本，工具定义另外单独扫：命中任意一条 `always_suspect` 模式 → `suspect`。每个模式都会跑，每处匹配都会逐个走一遍（保留最近的 8 处；同一个模式匹配超过 64 次后会往后跳，开销有上限），原因里写的是列表中排在最前面的那个模式。模式是 **PCRE**，通过 `ctx.re_find(subject, pattern, init)` 做大小写不敏感的匹配，它返回匹配到的字节区间（从 1 开始计数的闭区间 `from, to`），或者只返回一个真值；有了区间，就能把命中处放进送审窗口。OpenResty、Kong 和 APISIX 注入的是带 `"ijo"` 的 `ngx.re.find`（不开 UTF 的 PCRE：`\s` 只算 ASCII 空白，`.` 是一个字节），spec 里注入 lrexlib-pcre2，JavaScript core 则把每个模式翻译成一个在文本字节上运行、结果与之相同的 RegExp（非 ASCII 字面量按字节匹配，`i` 只折叠 ASCII）。`(?i)` 这类内联标志和占有量词翻译不过去，写模式时别用；`core/golden/rules.json` 为每条模式准备了一个命中样本，两边行为一旦不一致，就会有一个具名用例失败。所有适配器共用同一份规则文件。匹配器出错时（PCRE 的 JIT 栈用尽、模式编译不过），这个模式按命中算，只记一次日志：运维人员写坏的模式会偏向送审，而不是悄悄漏掉。如果没有注入匹配器，这一步会跳过，只告警一次，由长度检查单独决定（失败放行）。
