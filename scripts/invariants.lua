...
rule("rule-parity", function(r)
  ...
  -- watch_paths, text_fields and tool_fields: the same entries in the same
  -- order (a route left out of one copy is "path not watched" on that
  -- runtime only; the order of the fields is the order of the judged text).
  -- resolve() fills in the same text_fields and tool_fields for an inline
  -- rule that lists none.
  local function strings(s)
    local out = {}
    for v in (s or ""):gmatch('"([^"]*)"') do out[#out + 1] = v end
    return out
  end
  for _, k in ipairs({ "watch_paths", "text_fields", "tool_fields" }) do
